...
Run `pwd` in the Terminal and `getwd()` in the R Console. Are they the same? Why would a command using `data/file.csv` might work in one place but fail in the other.
:::
...
The command names the program, arguments tell it what to work on, and flags (also called options) modify its behaviour. For example:

```bash
grep -in "robin" data/bird_notes.txt
```
Here, `grep` is the command, `"robin"` and `data/bird_notes.txt` are arguments, `-i` means “ignore upper/lower case,” and `-n` adds line numbers.
...
For this lab, first move into the folder containing this file. Replace the example path with the path on *your* computer. If you are using an RProject, you will probably already be in the right place (nifty, right?) - check with `pwd` before navigating somewhere.
